 ...
 */
const analyze = (v) => {
  if (!v) return { score: 0, label: '입력 없음', tone: 'bg-zinc-200 dark:bg-zinc-800' };
  let s = 0;
  if (v.length >= 8) s += 1;
  if (v.length >= 12) s += 1;
  const cats =
  ...
  s += Math.max(0, cats - 1);
  s = Math.min(4, s);
  if (s <= 1) return { score: s, label: '매우 약함', tone: 'bg-rose-500' };
  if (s === 2) return { score: s, label: '약함', tone: 'bg-amber-500' };
  if (s === 3) return { score: s, label: '보통', tone: 'bg-lime-500' };
  return { score: s, label: '강함', tone: 'bg-cyan-500' };
};
