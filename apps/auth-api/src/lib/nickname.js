 ...
 *   - max base retry = 5 → 최악 99 * 5 = 495 회 DB 조회. 운영 케이스에선 사실상 1-2 회.
...
/**
 * @typedef {{ user: { findUnique: (args: { where: { nickname: string } }) => Promise<unknown> } }} NicknameLookup
 */
...
export const findAvailableNickname = async (
  ...
  for (let attempt = 0; attempt < MAX_BASE_RETRY; attempt += 1) {
    ...
    const baseHit = await prismaLike.user.findUnique({ where: { nickname: base } });
    if (!baseHit) return base;
    // 2) suffix 2..MAX_SUFFIX
    for (let i = 2; i <= MAX_SUFFIX; i += 1) {
      const candidate = `${base}${i}`;
      // eslint-disable-next-line no-await-in-loop
      const hit = await prismaLike.user.findUnique({ where: { nickname: candidate } });
