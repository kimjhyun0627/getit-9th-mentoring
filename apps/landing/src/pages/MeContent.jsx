...
export const MeContent = ({ user, displayName }) => {
  ...
  const landingMeUrl = 'https://get-it.cloud/me';
  // useSession 이 이미 blank 문자열을 null 로 정규화하지만, 안전망으로 한 번 더.
  const nicknameMissing = !user.nickname || user.nickname.trim().length === 0;
        ...
        >
          ...
          {schoolVerified ? (
            ...
            >
              ...
              <span className="text-zinc-900 dark:text-zinc-50">{user.studentId ?? '—'}</span>
            </p>
          ) : (
            <div className="mt-2">
              <p
