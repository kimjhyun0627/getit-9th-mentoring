 ...
 *
 * URL 빌딩 규칙:
 *  - `VITE_AUTH_URL` 끝의 `/` 는 제거 (`https://auth.get-it.cloud/` 도 안전하게 받기, CR #532)
 *  - `here` 가 빈 문자열이면 `?redirect=` 자체를 생략 (Gemini #532)
 *
 * SSR 안전: `typeof window === 'undefined'` 시 redirect 생략. shelf-web 은 CSR 전용이라
 * 실제 SSR 환경은 없지만 테스트 mock + 안전성 차원에서 guard.
 *
 * @returns {JSX.Element}
 */
export const RequireSignIn = () => {
  // import.meta.env 는 빌드 타임에 치환됨. 빈 문자열도 미설정으로 간주 → || 연산자.
  // 끝 슬래시 제거 — `https://auth.get-it.cloud/` 같은 입력도 `//login` 으로 깨지지 않게.
  const rawAuthBase = import.meta.env?.VITE_AUTH_URL || 'https://auth.get-it.cloud';
  ...
  return (
    ...
    >
      ...
      <h3
        ...
        className="font-display tracking-hero text-2xl font-black leading-[1.1] md:text-3xl"
      >
