...
- Project / ProjectMember / BoardColumn / Card 4 테이블 (User는 auth-api 공유 — board-api 스키마엔 미포함)
...
index: `(projectId, order, id)` — 동일 `order` 충돌 시 `id` 보조 정렬키로 결정적 순서 보장.
...
index: `(columnId, order, id)` — 동일 `order` 충돌 시 `id` 보조 정렬키로 결정적 순서 보장 / `assigneeId`.
...
| 스크립트                | 설명                                                                                                                                                                |
| :---------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `test`                  | `vitest run` — 현재는 Prisma schema 가드 (DB 연결 X). 후속 #46/#47/#48 에서 Express 라우트가 들어오면 **Vitest + supertest** 기반 API 통합 테스트로 확장 (TDD 필수) |
